...
function PhotocardListings() {
  const [pcListings, setPcListings] = useState([]);
  const LISTING_API_URL = "http://localhost:5000/api/categories/photocards/";

  useEffect(() => {
    axios.get(LISTING_API_URL).then((response) => setPcListings(response.data));
  }, []);
...
export default PhotocardListings;
