...
function BinderColbukListings() {
  const [bcbListings, setBcbListings] = useState([]);
  const LISTING_API_URL = "http://localhost:5000/api/categories/bindercolbuk/";

  useEffect(() => {
    axios
      .get(LISTING_API_URL)
      .then((response) => setBcbListings(response.data));
  }, []);
  ...
  return (
    <body>
      ...
      <div className="cat-listings-div">
        <h1>Binders and Collect Books</h1>
        <div className="pup-listings-map-item">
          {bcbListings.map((listing) => (
...
export default BinderColbukListings;
